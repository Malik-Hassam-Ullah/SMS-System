...
import { Filter, Download, FileText, Loader2, Search, Trophy, Award, AlertCircle, Printer, Eye, CheckCircle2, XCircle, ArrowLeft, School } from 'lucide-react';
...
export default function MarksViewPage() {
  ...
  useEffect(() => {
    const fetchInit = async () => {
      try {
        const [clsRes, exmRes, branchRes] = await Promise.all([
        ...
        if (!selectedExam && exmList.length > 0) {
          setSelectedExam(exmList[0].id);
        }
      } catch (e) {
        console.error(e);
      } finally {
  ...
  const handleOpenResultCard = async (student) => {
    try {
      ...
      toast.error('Failed to fetch student result card details.');
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900 flex items-center gap-3">
            ...
            Class-wise Examination Results & Rankings
          </h1>
          <p className="text-sm text-slate-500 mt-1">
            Consolidated subject-wise results, automatic percentage and position calculations, and Top 5 rankers.
          </p>
        </div>
              ...
              <thead>
                <tr className="bg-slate-900 text-white font-bold uppercase tracking-wider text-[11px]">
                  ...
                  {(reportData?.subjects || []).map(sub => (
                    <th key={sub} className="p-3 text-center whitespace-nowrap">
                      {sub}
                    </th>
                  ))}
